...
```
Base Layer:     Dark canvas (#12121A) or warm off-white (#F5F5F0)
                NEVER pure #000000 or #FFFFFF
Content Layer:  Z1 surface (#1C1C22 dark / pure white light)
Brand Accent:   Neon Green: #BAFD50 (CTA, selected state)
                Aurora Purple: #A66CFF (glow effects only)
Semantic:       Fire Red: #F65555 (calories, fat)
                Energy Yellow: #FFD556 (energy, protein)
                Recovery Blue: #87DCFB (hydration, rest)
Surface Glass:  rgba(255,255,255, 0.06) on dark
                rgba(0,0,0, 0.02) on light
```

### Usage Rules
- **Multiple accent colors are OK** — but each must represent a distinct data category (see `bento-color-playbook.md` Decision 1)
- **All accents must share the same saturation filter:** light mode = "add milk" (pastel), dark mode = "electrify" (neon). Never mix filters.
- **80/20 budget:** Accents compressed into ≤20% of screen area. 80% stays neutral.
- **Vivid card backgrounds:** If a card has a bold color fill, ALL inner content goes monochrome (black or white only).
- **Gradients:** Subtle, max 2 stops, same hue family (e.g., red → yellow for "fire", not red → blue).
- **Adjacent accents touching:** Blur/diffuse boundaries — no hard-edged color tiles.
