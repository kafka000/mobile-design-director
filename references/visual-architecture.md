...
|-----------|------|----------|
| **80% Neutrals** | Structure — backgrounds, containers, text, dividers | `bg-neutral-50`, `text-neutral-900`, `border-neutral-200` |
| **20% Intent** | Data categories, state, and CTAs | Chart bars, progress rings, category tags, CTA buttons |

High-saturation accent colors serve **data and action**, not decoration. Permitted uses:
- Data category encoding (e.g., protein = gold, carbs = purple, fat = red)
- Primary CTA buttons ("Start Workout")
- State indicators (heart rate warning, completion rings)
- Progress and metric visualizations

> **Critical constraint:** Every color in the 20% budget must have a "job" — a data category, state, or action it uniquely represents. See `bento-color-playbook.md` Decision 1 for the full litmus test. Color without a data label is banned.
...
```
Z0 (Base):       #12121A    — global dark canvas (bg-base-dark)
Z1 (Surface):    #1C1C22    — card / container (bg-surface-dark-1)
Z2 (Elevated):   #2A2A32    — floating / selected state (bg-surface-dark-2)
Light Panel:     #F5F5F0    — oatmeal white for light sections (bg-panel-light)
```

> **Important:** Never use pure #000000 (too dead) or #FFFFFF (too harsh) as base. The slight blue-gray tint in #12121A provides a warm, breathable dark canvas.

Additionally, accent colors in dark mode must:
- Use **"Cyberpunk Neon" filter** — high saturation, luminous against dark base
- Maintain WCAG AA contrast (4.5:1 minimum)
- Stay within small surface areas (like neon signs in a dark alley)
...
| Color allocation | Accent used for decoration (no data "job") | 80% neutrals, 20% data-mapped intent |
| Multi-color | Random colors for aesthetics | Each color = a data category; same saturation filter |
| Bento grid | Content floating on raw background | Every data group boxed in rounded container |
...
| Dark mode depth | Color shifts for elevation | Luminance steps (#12121A → #1C1C22 → #2A2A32) |
| Vivid card bg | Colorful card + colorful content inside | Bold card bg → ALL inner content monochrome |
