...
6. **80/20 Color Rule:** 80% neutral tones for structure, 20% intentional color — but ONLY where each color maps to a distinct data category or state. Color without a data "job" is banned.
...
11. **Bento Grid Layout:** All content goes into rounded containers — no data floats on raw background. The brain parses boxes instantly; scattered elements cause cognitive fatigue.

12. **Multi-Color Decision Filter:** Adding a color? Pass this checklist: (1) Does it represent a new data category? (2) Does it match the existing saturation filter? (3) Does it stay within the 20% color budget? (4) If the card background is vivid, is ALL inner content monochrome? (5) If multiple accents touch, are boundaries blurred/diffused?
...
- **Bento UI + multi-color decisions (layout boxes, color "job" filter, saturation matching):** [references/bento-color-playbook.md](references/bento-color-playbook.md)
...
- **Visual Architecture Audit:** Validate against the 5 pillars in `visual-architecture.md`:
  1. Color Token pipeline (three-layer architecture, 80/20 allocation, state management)
...
- **Bento + Color Decisions:** Validate against the playbook in `bento-color-playbook.md`:
  1. Bento Grid: Every content group boxed in rounded containers
  2. Multi-Color Filter: Every color has a data "job"; same saturation filter; 80/20 budget
  3. Diva Rule: Vibrant card background → monochrome content inside
  4. Diffusion Rule: Adjacent accents → blur/dissolve boundaries
